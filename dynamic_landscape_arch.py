...
filename = 'archived_sources.pkl'
with open(filename, 'rb') as f:
        archived_pop = pickle.load(f)

n_archived = archived_pop.n_individuals
...
for ss in range(n_samples):
    ...
    if poptype == 'sources':
        imm_pop = SourcePopulation(n_imm, n_sources, img_dim)
    elif poptype == 'pixels':
        imm_pop = PixelPopulation(n_imm, img_dim, n_filters)
    imm_pop.generate_random_population()
    pop = join_populations([imm_pop, archived_pop])
...
filename = 'draft01_dynamic_landscape_' + poptype + '_arch5.pkl'
