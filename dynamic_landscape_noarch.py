...
# ---- Simulation starts
variant = 2
# variant == 1 --> initial random population
# variant == 2 --> last gen population + random
flag = 0
for ss in range(n_samples):
    ...
    if variant == 1 or flag == 0:
        if poptype == 'sources':
            pop = SourcePopulation(n_archived + n_imm, n_sources, img_dim)
        elif poptype == 'pixels':
            pop = PixelPopulation(n_archived + n_imm, img_dim, n_filters)
        pop.generate_random_population()
        flag = 1
    elif variant == 2:
        if poptype == 'sources':
            pop_add = SourcePopulation(n_archived + n_imm - n_individuals, n_sources, img_dim)
        elif poptype == 'pixels':
            pop_add = PixelPopulation(n_archived + n_imm - n_individuals, img_dim, n_filters)
        pop_add.generate_random_population()
        pop = join_populations([pop_add, pop])
...
filename = 'draft01_dynamic_landscape_' + poptype + '_noarch5_var2.pkl'
